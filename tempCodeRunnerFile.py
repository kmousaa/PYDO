...
def do_ai(instruction, list_name, current):
    ...
    rearrange_triggers = ["rearrange", "rearange", "reorder", "re-order", "sort", "prioritise", "prioritize","order"]
    ...
    # If the return is a list, its a re-ordering most likely 

    if isinstance(data, list):
        print("LESGO!")

        # Re order the list based on the new order given by AI, which is a list you should replace the current list with — but keep the completion status of each task the same as before, just reorder them based on the new order given by AI 
